#!/bin/vbash

set interfaces ethernet eth0 description 'WAN'
set interfaces ethernet eth0 address 'dhcp'
set interfaces ethernet eth0 hw-id '98:fa:9b:65:f4:50'
set interfaces ethernet eth0 mtu '1508'
set interfaces ethernet eth0 vif 6 description 'Internet'
set interfaces ethernet eth0 vif 6 mtu '1508'

set interfaces ethernet eth1 description 'LAN'
set interfaces ethernet eth1 hw-id '90:e2:ba:36:e2:08'
set interfaces ethernet eth1 address '10.66.0.1/24'
set interfaces ethernet eth1 vif 10 address '10.66.1.1/24'
set interfaces ethernet eth1 vif 10 description 'SERVERS'
set interfaces ethernet eth1 vif 20 address '10.66.2.1/24'
set interfaces ethernet eth1 vif 20 description 'TRUSTED'
set interfaces ethernet eth1 vif 30 address '192.168.50.1/24'
set interfaces ethernet eth1 vif 30 description 'GUEST'
set interfaces ethernet eth1 vif 40 address '10.66.3.1/24'
set interfaces ethernet eth1 vif 40 description 'IOT'

set interfaces ethernet eth1 description 'LAN'
set interfaces ethernet eth2 hw-id '90:e2:ba:36:e2:09'
set interfaces ethernet eth2 address '10.66.0.2/24'
set interfaces ethernet eth2 vif 10 address '10.66.1.2/24'
set interfaces ethernet eth2 vif 10 description 'SERVERS'
set interfaces ethernet eth2 vif 20 address '10.66.2.2/24'
set interfaces ethernet eth2 vif 20 description 'TRUSTED'
set interfaces ethernet eth2 vif 30 address '192.168.50.2/24'
set interfaces ethernet eth2 vif 30 description 'GUEST'
set interfaces ethernet eth2 vif 40 address '10.66.3.2/24'
set interfaces ethernet eth2 vif 40 description 'IOT'

set interfaces ethernet eth1 description 'LAN'
set interfaces ethernet eth3 hw-id '90:e2:ba:36:e2:0a'
set interfaces ethernet eth3 address '10.66.0.2/24'
set interfaces ethernet eth3 vif 10 address '10.66.1.3/24'
set interfaces ethernet eth3 vif 10 description 'SERVERS'
set interfaces ethernet eth3 vif 20 address '10.66.3.3/24'
set interfaces ethernet eth3 vif 20 description 'TRUSTED'
set interfaces ethernet eth3 vif 30 address '192.168.50.3/24'
set interfaces ethernet eth3 vif 30 description 'GUEST'
set interfaces ethernet eth3 vif 40 address '10.66.3.3/24'
set interfaces ethernet eth3 vif 40 description 'IOT'

set interfaces ethernet eth4 hw-id '90:e2:ba:36:e2:0b'

set interfaces wireguard wg01 address '10.66.5.1/24'
set interfaces wireguard wg01 description 'WIREGUARD'
set interfaces wireguard wg01 peer ot-framework allowed-ips '10.66.5.2/32'
set interfaces wireguard wg01 peer ot-framework persistent-keepalive '15'
set interfaces wireguard wg01 peer ot-framework public-key 'rEU7DZLDS6BNd+pe3GgbbqvLJBDfQLDSFdjzXOczpDg='
set interfaces wireguard wg01 peer oliver-pixel5 allowed-ips '10.66.5.3/32'
set interfaces wireguard wg01 peer oliver-pixel5 persistent-keepalive '15'
set interfaces wireguard wg01 peer oliver-pixel5 public-key 'y41/A4zqAqzr9KHKEkMXBhqoQH0lz62/tSi8VFtRWS8='
set interfaces wireguard wg01 peer oliver-pixel9 allowed-ips '10.66.5.4/32'
set interfaces wireguard wg01 peer oliver-pixel9 persistent-keepalive '15'
set interfaces wireguard wg01 peer oliver-pixel9 public-key 'A3bNqkt5Ys6QNuPHzNc8tX9+x+j0WO0Wp57ZOVKHvFQ='
set interfaces wireguard wg01 port '51820'
set interfaces wireguard wg01 private-key "${SECRET_WIREGUARD_PRIVATE_KEY}"
